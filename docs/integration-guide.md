...
1. **mpr-ui assets** – load `mpr-ui.css` and `mpr-ui.js` from the CDN or from a local build. The `<mpr-*>` Web Components DSL works without Alpine; the deprecated advanced factories (`mprSiteHeader`, `mprFooter`, `mprThemeToggle`, etc.) are only supported during the migration window and should no longer be part of new integrations.
