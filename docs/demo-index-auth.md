...
- `mpr-ui` calls `initAuthClient({ baseUrl, onAuthenticated, onUnauthenticated })` and uses `requestNonce`, `exchangeGoogleCredential`, and `logout` when those helpers are available.
- The helper:
