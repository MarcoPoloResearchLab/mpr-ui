...
- [x] [B013] (P0) Bind browser Google sign-in attempts to a fresh nonce claim without idle GIS work.
  ...
  Resolved 2026-06-07: moved GIS initialization and nonce issuance into the explicit sign-in attempt, with credential callbacks closing over the attempt nonce and rejecting callbacks that arrive without it. Header and `<mpr-login-button>` now render first-party sign-in triggers on public bootstrap and start the nonce-bound GIS prompt only on user action. Updated unit/browser/E2E coverage for four-hour idle pages, stale callbacks, endpoint rebinding, missing nonce rejection, and click-time GIS initialization. Tests: focused Playwright regressions; `make ci`.

- [x] [B012] (P0) Long-idle auth surfaces must not emit console errors or hide auth failures.
...
- [x] [B008] (P0) Apps need `mpr-ui` to own the post-sign-in redirect and transition handoff.
...
- [x] [B009] (P0) Long-idle Google buttons can exchange a nonce that is already expired.
