...
- [x] [B044] (P1) Session-first `<mpr-login-button>` renders an unstyled nested browser button and discards the consumer CTA presentation.
  ...
  Requirements:
  - Keep the canonical click-time sequence: no Google Identity initialization, TAuth nonce request, `/me`, or `/auth/refresh` probe before an explicit sign-in action; the action must still bind the Google prompt to its fresh nonce.
