...
- Auth provider chooser demo follow-up: added a browsable `demo/auth-provider-chooser.html` page with compact all-provider, Google-only, and Google/email examples plus a provider event log, linked it from the shared demo navigation, and updated demo docs. Tests: `npm run test:unit`; `npm run test:e2e`; `npx --yes --package typescript@5.9.2 tsc --noEmit --allowJs --checkJs --lib ES2020,DOM --module nodenext --moduleResolution nodenext --target ES2020 demo/auth-provider-chooser.js`; live Playwright smoke against `http://127.0.0.1:4177/demo/auth-provider-chooser.html`.
- Auth provider chooser visual follow-up: added compact decorative Apple, Google, and email marks to provider actions, preserved accessible labels, and documented that provider marks are UI cues rather than final Google/Apple auth-branding certification. Tests: `npm run test:unit`; `npx --yes --package typescript@5.9.2 tsc --noEmit`; `npx playwright test tests/e2e/auth-provider-chooser.spec.js`; `npm run test:e2e`; live Playwright smoke against `http://127.0.0.1:4177/demo/auth-provider-chooser.html`.
- Static demo preview fix: added an explicit `http://127.0.0.1:4177` / `http://localhost:4177` environment to `demo/config-ui.yaml` so the documented lightweight Python server upgrades the root demo header/footer instead of halting before bundle load, documented the two preview modes, and relaxed the optional live demo-stack smoke to accept `/index.html` as the hub URL. Tests: `npm run test:unit`; `npx --yes --package typescript@5.9.2 tsc --noEmit`; live Playwright smoke against `http://127.0.0.1:4177/`; `MPR_UI_DEMO_BASE_URL=http://127.0.0.1:4177 npx playwright test tests/e2e/demo-stack.spec.js`.
- Auth provider chooser icon-row variant: added `variant="icon-row"` for horizontal icon-only Apple/Google/email buttons with preserved accessible labels, demo coverage, README/custom-elements/integration docs, and `mpr-integration` skill contract notes. Tests: `git diff --check`; `npx --yes --package typescript@5.9.2 tsc --noEmit`; `npm run test:unit`; `npx playwright test tests/e2e/auth-provider-chooser.spec.js`; `npx --yes --package typescript@5.9.2 tsc --noEmit --allowJs --checkJs --lib ES2020,DOM --module nodenext --moduleResolution nodenext --target ES2020 demo/auth-provider-chooser.js`; `npm run test:e2e`.
- Auth provider chooser square icon-row correction: tightened `variant="icon-row"` from wide row cells to fixed horizontal square icon buttons, mirrored the rules in the injected stylesheet, updated docs/skill language, and strengthened Playwright assertions to require square button geometry. Tests: `git diff --check`; `npx --yes --package typescript@5.9.2 tsc --noEmit`; `npx playwright test tests/e2e/auth-provider-chooser.spec.js`; `npm run test:unit`; `npx --yes --package typescript@5.9.2 tsc --noEmit --allowJs --checkJs --lib ES2020,DOM --module nodenext --moduleResolution nodenext --target ES2020 demo/auth-provider-chooser.js`.
- Auth provider chooser compact/cache correction: made icon-row styling apply from the public `variant="icon-row"` attribute as well as reflected component state, reduced the buttons to 36px square controls, added the shared `npm run demo:serve` no-store static server instead of per-page cache busting, and documented that as the local preview path. Tests: `git diff --check`; `node --check scripts/serve-demo.mjs`; `npx --yes --package typescript@5.9.2 tsc --noEmit`; `npm run test:unit`; `npx playwright test tests/e2e/auth-provider-chooser.spec.js`; `npm run test:e2e`; no-store header smoke for `/`, `/mpr-ui.css`, and `/mpr-ui.js`; live provider chooser smoke confirmed 36px square row buttons. Note: optional demo-stack smoke on port 4277 failed because `demo/config-ui.yaml` is intentionally configured for the documented 4177 origins.
- Auth provider chooser outline consistency: removed email-only dashed outline styling and Apple-only border color override so provider buttons share the same outline treatment regardless of provider count, then added unit/static and Playwright assertions for uniform icon-row border style, color, and width. Tests: `git diff --check`; `npx --yes --package typescript@5.9.2 tsc --noEmit`; `npm run test:unit`; `npx playwright test tests/e2e/auth-provider-chooser.spec.js`; `npm run test:e2e`.
- Auth provider chooser provider-update reconciliation: cleared stale `data-mpr-auth-provider-selected` state when an observed `providers` update removes the selected provider, preserved still-available selections, and collapsed stale email expansion with focused regression coverage. Tests: `git diff --check`; `npx --yes --package typescript@5.9.2 tsc --noEmit`; `npm run test:unit`; `npx playwright test tests/e2e/auth-provider-chooser.spec.js`.
...
- F007: added explicit password and account endpoint config, shared `<mpr-password-auth>` and `<mpr-account-panel>` components, one lifecycle-safe TAuth action layer, Google identity linking, and secret-safe component/account events.
- F007: updated the demo TAuth policy, all password/account demos, public docs, and the adjacent `mpr-integration` contracts. Tests: `make ci` with 189 Node tests and two 96-test browser runs; browser coverage reported 76.91% lines, 98.75% branches, and 81.03% functions.
