...
This archive preserves resolved non-recurring issue history from `.mprlab/ISSUES.md`.

Archived 2026-09-01 by M400R.
...
- [x] [B008] (P1) mpr-ui: `base-class` utilities like `mt-auto` are ineffective for flexbox layout when `sticky="false"`.
  ### Summary
  ...
  ### Affected version
  mpr-ui v3.8.2
  Resolved 2026-09-01: MU-372 mirrors non-chrome `base-class` values to the host when `sticky="false"`.
  Validation: Three focused unit tests and the footer layout Playwright test passed.
...
- [x] [B043] Release and publish depended on sibling agentSkills/gitrelease. Vendor the canonical core bundle, route those targets locally, preserve jsDelivr deploy, and validate the observable Make contract.
  Resolved 2026-09-01: Repository-owned release scripts now prepare and publish releases without the sibling skill.
  Validation: The three release contract tests passed and preserved the jsDelivr deploy target.
...
- [x] [F008] (P1) Add config-driven Apple Sign In redirect-provider support to `mpr-ui`.
  ...
  Resolution: Added the explicit Google and Apple provider map. Apple now uses a validated TAuth redirect and the shared session lifecycle.
  Added shared controls, diagnostics, test helpers, demos, and integration contracts. Apple-only tenants do not require Google settings.
  Validation: `make ci` passed 180 Node tests, 95 browser coverage scenarios, and 95 Playwright tests on 2026-09-01.

- [x] [F007] (P1) Add config-driven TAuth email/password authentication and account-management forms to `mpr-ui`.
  ...
  - `make ci` passes with the new tests.
  Resolution: Added explicit password-provider, password-endpoint, and account-endpoint configuration with no hidden paths. Added shared password and account components on one auth controller, including Google linking and lifecycle-safe TAuth actions.
  Added complete demo, documentation, `mpr-integration` contract, Node, and Playwright coverage. Browser-visible events, attributes, storage, rendered state, profiles, and logs exclude submitted secrets and challenge tokens.
  Validation: `make ci` passed 189 Node tests, 96 browser coverage scenarios, and 96 Playwright tests on 2026-09-01. Browser coverage reported 76.91% lines, 98.75% branches, and 81.03% functions for `mpr-ui.js`.
